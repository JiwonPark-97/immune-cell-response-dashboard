...
def populate_db(conn, data):
    ...
    conn.executemany("INSERT INTO projects (project_code) VALUES (?)", [(code,) for code in project_codes])
    
    project_ids = dict(conn.execute("SELECT project_code, id FROM projects").fetchall())

    # Subjects
    subjects = data[["project", "subject", "condition", "age", "sex", "treatment", "response"]].drop_duplicates()
    subject_rows = [
        (
    ...
    conn.executemany(
        "INSERT INTO subjects (project_id, subject_code, condition, age, sex, treatment, response) VALUES (?, ?, ?, ?, ?, ?, ?)", 
        subject_rows)
    ...
    conn.executemany(
        "INSERT INTO samples (sample_code, sample_type, subject_id, time_from_treatment_start) VALUES (?, ?, ?, ?)",
        sample_rows
    )
    ...
    conn.executemany(
        "INSERT INTO cell_counts (sample_id, population_id, cell_count) VALUES (?, ?, ?)",
        count_rows
    )
